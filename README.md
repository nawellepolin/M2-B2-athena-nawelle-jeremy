...
> - **Sync jeudi (binôme)** : nomme ton repo `M2-B2-athena-<prénom1>-<prénom2>`
>   sur le compte de l'un de vous deux. L'autre est invité comme collaborateur.
> - **Async vendredi (perso)** : **fork** le repo binôme sous
...
> Discord** (`fil-M2-B2`) jeudi 9h — tout le monde reçoit le **même** jeu
...
### Jeudi sync (2h15) — binôme

1. **Setup binôme** (15 min) — choix du binôme, repo commun, conventions
   `Co-authored-by:`, switch driver/navigator
2. **Audit éthique croisé** (1h30) — le cœur du brief :
   - **a. Chacun·e de son côté** (20 min, **sans vous concerter**) — calculez le
     disparate impact (règle des 4/5) sur la **même** variable sensible. Prenez-en
     une **où un choix se pose** — `age` (quel seuil jeune/senior ?) ou une
     variable avec des `?` (comment traiter les manquants ?) — pas une binaire
     déjà tranchée. Ne comparez pas encore vos résultats.
   - **b. Confrontez vos deux chiffres** (15 min) — s'ils **diffèrent**, trouvez
     pourquoi : définition de l'issue favorable, groupe de référence, traitement
     des manquants, seuil retenu. S'ils sont identiques, vérifiez que c'est pour
     les **mêmes raisons**.
   - **c. Réconciliez puis étendez** (55 min) — actez une **méthode commune**,
     puis calculez le DI sur **≥ 3 variables sensibles + une intersection**.
   > 🎯 Leçon à écrire noir sur blanc dans la datasheet (§ biais) : **un
   > disparate impact n'est pas un chiffre objectif — c'est une décision
   > méthodologique qui doit être documentée.** Un jury (et un DPO) n'attend pas
   > le chiffre, il attend la **justification** du chiffre.
3. **Datasheet binôme** (30 min) — 7 sections Gebru, signée duo (dont la
   méthode de DI réconciliée en § biais)
4. **Tour de table 11h30** — restitution duo 5 min × 4 paires. **Comparez vos DI
   entre paires** : mêmes données (graine fixe), méthodes différentes → écarts ?

### Async vendredi matin (6h) — individuel
...
4. En binôme : si vous bloquez à 2, **switchez** driver/navigator —
   souvent ça débloque.
5. Demande en direct jeudi sur Discord — `fil-M2-B2`.